...
class SentimentLDAGibbsSampler:
    ...
    def __init__(self, numTopics, alpha, beta, gamma, seed=None, numSentiments=2):
        """
    ...
    def sampleFromDirichlet(self, alpha):
        """
    ...
    def sampleFromCategorical(self, theta):
        ...
        return self.rng.multinomial(1, theta).argmax()


    def word_indices(self, wordOccuranceVec):
        """
    ...
    def _initialize_(self, reviews, sentiFun, saveAs=None, saveOverride=False):
        ...
        for i, word in enumerate(self.vectorizer.get_feature_names()):
            (posScore, negScore) = sentiFun(word)
        ...
        for d in range(numDocs):
            topicDistribution = self.sampleFromDirichlet(alphaVec)
            sentimentDistribution = np.zeros((self.numTopics, self.numSentiments))
            for t in range(self.numTopics):
                sentimentDistribution[t, :] = self.sampleFromDirichlet(gammaVec)
            for i, w in enumerate(self.word_indices(self.wordOccuranceMatrix[d, :])):
                t = self.sampleFromCategorical(topicDistribution) #random topic
                s = self.sampleFromCategorical(sentimentDistribution[t, :]) #random sentiment
    ...
    def getTopKWordsByLikelihood(self, K):
        ...
        for t in range(self.numTopics):
            for s in range(self.numSentiments):
                ...
                ret.append((t, s, [vocab[i] for i in topWordIndices]))
        return ret
    ...
    def run(self, reviews, sentiFun, maxIters=30, saveAs=None, saveOverride=False):
        """
        Runs Gibbs sampler for sentiment-LDA
        """
        self._initialize_(reviews, sentiFun, saveAs, saveOverride)
        numDocs, vocabSize = self.wordOccuranceMatrix.shape
        for iteration in trange(maxIters):
            for d in range(numDocs):
                for i, v in enumerate(self.word_indices(self.wordOccuranceMatrix[d, :])):
                    t = self.topics[(d, i)]
                    ...
                    if v in self.priorSentiment:
                        ...
                        t = self.sampleFromCategorical(probabilities_ts[:, s])
                    else:
